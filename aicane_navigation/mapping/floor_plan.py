...
class FloorPlan:
    ...
    def __init__(self):
        ...
        self.corridor_y = 1312.5  # 복도 중앙 y 좌표
        # 방 정보
    ...
    def generate_simple_path(
        ...
        from_room: str,
        to_room: str,
        waypoint_gap: float = 50.0,
        *,
        spacing_cm: float | None = None,
    ):
            """
            방 간 이동을 위한 단순한 경로 생성 함수.
            복도(y = corridor_y)를 따라 직선 이동 후 방 안으로 진입하는 방식.

            Args:
                from_room: 출발 방 이름 (예: '101호')
                to_room: 도착 방 이름 (예: '107호')
                waypoint_gap: 기본 웨이포인트 간격(cm)
                spacing_cm: 과거 버전 호환용 키워드 (지정하면 waypoint_gap 대신 사용)

            Returns:
                list[(x_cm, y_cm)] : 경로 웨이포인트 리스트
            """
            # spacing_cm이 들어오면 waypoint_gap을 교체 (과거 테스트와 호환)
            if spacing_cm is not None:
                waypoint_gap = spacing_cm
